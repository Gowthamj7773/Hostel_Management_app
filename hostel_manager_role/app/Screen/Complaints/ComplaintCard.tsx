...
const pending = require('../../../assets/icons/clock.png')
    ...
    // Complaint Card 
export default function ComplaintCard({index,name,hostel,floor,date,issueType,issue,status ,role}){
    ...
    <Pressable onPress={()=>setSeeMore(prev => !prev)} className="bg-white p-4 mt-3 mx-5 rounded-lg shadow border border-gray-200">
        <View className="flex-row justify-between items-start">
            <View>
                <Text className="text-gray-900 font-semibold">
                    {index}. <Text>{name}<Text> {role}</Text></Text>
                </Text>
        ...
        <Text className="mt-3 text-gray-900 font-semibold">{issueType}</Text>
        {
            ...
            <Text className="mt-1 text-gray-600 text leading-5">{issue.slice(0,30)}<Text>...</Text></Text>
        }
        {
            seeMore ? 
